<!-- Filename: J4.x:Joomla_Standard_Font_Awesome_Fonts / Display title: Шрифты Font Awesome -->

## Как использовать

CMS Joomla! поставляется с собственным набором шрифтов Font Awesome. Эти иконки доступны по умолчанию для использования в шаблонах *Cassiopeia* и *Atum*.

Иконки, которые сопоставлены в файле `/media/system/scss/_icomoon.scss`, можно вызвать с помощью тега `name-of-icon">` и пробела `{space}` `</span>`. Например:

```html
<span class="icon-joomla"> </span>
```

Покажет иконку Joomla!: <span class="icon-joomla">&nbsp;</span>

Иконки, которые не сопоставлены, можно вызвать с помощью тега `<span class="far fa-name-of-icon">` и пробела `{space}` `</span>`. Пример:

```php
<i class="fa fa-adjust"></i>
```

Покажет иконку "настройка": <i class="fa fa-adjust"></i> 

## Размер шрифта

Поскольку иконки представлены в виде шрифтов, вы можете управлять их размером с помощью добавленного класса или атрибута style=. Конечно, вам нужно будет определить этот класс в вашем .css или .less файле стилей.
...
Вот иконка Joomla!, измененная в размере: <span class="icon-joomla" style="font-size:24px;">&nbsp;</span>
